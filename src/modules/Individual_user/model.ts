...
export namespace IndividualUserModel {
    export const userSchema = t.Object({
        ...
        street: t.Optional(t.String({ pattern: "^[a-zA-Z0-9]{1,256}$" })),
        city: t.Optional(t.String({ pattern: "^[a-zA-Z0-9]{1,128}$" })),
        state: t.Optional(t.String({ pattern: "^[a-zA-Z0-9]{1,128}$" })),
        country: t.Optional(t.String({ pattern: "^[a-zA-Z0-9]{1,128}$" })),
        addressProof: t.Optional(t.String({ format: "uri", description: "Address proof must be a valid URL", examples: ["https://example.com/address-proof.jpg"] })),
        dob: t.Optional(t.Date({ description: "Date of birth must be a valid date", examples: ["2000-01-01"] })),
        gender: CommonSchema.genderSchema,
        bvn: t.Optional(t.String({ pattern: "^[0-9]{11}$", description: "Bank Verification Number must be a valid number", examples: ["12345678901"] })),
